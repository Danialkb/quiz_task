...
class FinishQuizSessionCommand(UseCase):
    ...
    async def execute(self, quiz_session_id: UUID, user_id: UUID) -> QuizSessionFinishedResponse:
        ...
        quiz_session.bonus_points = await self.bonus_adder.add_bonus_points(quiz_session_id, user_id)
