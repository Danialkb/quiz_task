...
class FillGapValidation(AnswerValidationStrategy):
    async def validate(
        self, user_answer: UserAnswerCreateSchema, options: list[Option]
    ) -> tuple[bool, list[UUID]]:
        if len(user_answer.options) != 1:
            raise ServiceException(
                "Fill Gap question should contain exactly one option"
            )

        selected = user_answer.options[0]
        is_correct = any(
            option.id == selected and option.is_correct for option in options
        )
